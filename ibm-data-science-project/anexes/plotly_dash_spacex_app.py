...
app.layout = html.Div(
    children=[
        ...
        dcc.Dropdown(
            ...
            value="ALL",
            placeholder="Please select launch site here...",
            searchable=True,
        ),
        html.Br(),
...
def get_pie_chart(selected_site):
    if selected_site == "ALL":
        ...
        fig = px.pie(
            values=values, names=labels, title="Launch Success Rate for All Sites"
        )
    else:
        filtered_df = spacex_df[spacex_df["Launch Site"] == selected_site]
        success_count = filtered_df["class"].sum()
        failed_count = filtered_df["class"].size - success_count
...
def get_scatter_plot(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Launch Outcomes for All Sites')
    else:
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title=f'Launch Outcomes for {selected_site}')
    return fig
